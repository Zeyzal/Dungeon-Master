...
/mob/proc/ForcePickUpItem(atom/movable/ITEM)
	...
	weight+=ITEM.weight
	return(ITEM)
/mob/proc/DropItem(atom/movable/ITEM,UnEquip=1)if(ITEM.loc==src)
...
/mob/proc/ForceDropItem(atom/movable/ITEM,UnEquip=1)
	...
	if(ITEM.NoDropOnDeath)
		...
		return(1)
	return(ITEM)
/mob/proc/ToggleEquipItem(obj/Items/ITEM,MANUAL) switch(ITEM.suffix)
	if("(Equipped)") return(UnEquipItem(ITEM,MANUAL))
	if("(Carrying)") return(EquipItem(ITEM,MANUAL))
	else return(0)
/mob/proc/EquipItem(obj/Items/ITEM,MANUAL) if(UsesEquipment&&ITEM.loc==src&&ITEM.EquipmentType()&&ITEM.suffix=="(Carrying)")
...
/mob/proc/ForceEquipItem(obj/Items/ITEM)
	...
	return(ITEM)
/mob/proc/UnEquipItem(obj/Items/ITEM,MANUAL) if(ITEM.EquipmentType()&&ITEM.suffix=="(Equipped)"&&ITEM.loc==src)
